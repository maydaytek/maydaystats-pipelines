"""Cloud Run Job entrypoint for the NCAA women's volleyball pipeline.
...
Women's volleyball is a fall sport (roughly late August through the
national championship in mid-December). A daily run during the
off-season (January through August) will correctly fetch zero rows;
that's expected, not a bug - see fetch.py.
...
DATASET_ID = os.environ.get("BQ_DATASET", "ncaa_volleyball_women")
...
def main() -> None:
    ...
    if start and end:
        ...
        print(f"Fetching recent {SPORT} boxscores (rolling window)")
        df = fetch_recent(sport=SPORT)
